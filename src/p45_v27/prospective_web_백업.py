...
class ProspectiveOrbitService:
    ...
    def read(self) -> dict[str, Any]:
         state_files = list(self.prospective_root.glob("P45_TRIO_ORBIT_PROSPECTIVE_STATE_*.json"))
         if not state_files:
             raise RuntimeError("PROSPECTIVE_STATE_MISSING")
         self.state_path = max(state_files, key=lambda p: int(re.search(r"STATE_(\d+)\.json$", p.name).group(1)))
        ...
        pending = row["result_status"] == "PENDING"
        ...
        completed = [r for r in rows if r["result_status"] == "OUTCOME_RECORDED"]
        ...
        return {
            ...
            "current":{"target":target,"target_result_status":"AVAILABLE" if target_available else "PENDING","result_status":row["result_status"],
            ...
            "prospective":{"completed_rounds":len(completed),"pending_count":sum(r["result_status"]=="PENDING" for r in rows),
                           ...
                           "fixed_only_exact3":total("fixed_only_exact3_contribution"),"linked_only_exact3":total("linked_only_exact3_contribution"),"rows":rows},
    ...
    def record_outcome(self, target: int) -> dict[str, Any]:
        ...
        state=json.loads(self.state_path.read_text(encoding="utf-8-sig"));state["state_status"]="OUTCOME_RECORDED";state["result_status"]="OUTCOME_RECORDED";state["outcome_recorded_at"]=now
        self._safe_write(outcome_path,json.dumps(outcome,ensure_ascii=False,indent=2)+"\n",overwrite=False)
        self._write_log(rows);self._safe_write(self.state_path,json.dumps(state,ensure_ascii=False,indent=2)+"\n");self._audit("RECORD_OUTCOME",target,outcome_path);self._write_manifest()
        return outcome
    ...
    def seal_next(self, preview_sha256: str) -> dict[str, Any]:
        ...
        state={"state_status":"POST_SELECTION_PRE_OUTCOME","target_round":target,"source_round":target-1,"result_status":"PENDING",
               ...
               "sealed_record_sha256":sealed_sha,"future_leakage":0,"official_protected_changes":0}
        self._safe_write(self.state_path,json.dumps(state,ensure_ascii=False,indent=2)+"\n");self._audit("SEAL_NEXT",target,sealed_path);self._write_manifest()
