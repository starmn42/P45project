...
RESEARCH_MASTER_REL = Path("90_RESEARCH/P45_RESEARCH_MASTER_INDEX_003.md")
...
def _read_draws(path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        result.append({"round": int(row["round"]), "date": row.get("date", ""), "main": main, "bonus": bonus})
    return result
...
class ProspectiveOrbitService:
    ...
    def _research_status(self) -> dict[str, str]:
        """Read current numbered-experiment status from the authoritative Master."""
        text = (self.root / RESEARCH_MASTER_REL).read_text(encoding="utf-8-sig")
        required = {
        ...
        if not all(label in text and status in text for label, status in required.items()):
            raise RuntimeError("RESEARCH_MASTER_STATUS_MISMATCH")
        return {"latest_exp":"EXP-020","latest_exp_status":required["EXP-020"],"exp017":required["EXP-017"],
                "exp018_v1":required["EXP-018 V1"],"exp018_v2":required["EXP-018 V2"],"exp019":required["EXP-019"]}
    ...
    def read(self) -> dict[str, Any]:
        ...
        # The prospective ledger is append-only, so one target may have a pre-outcome
        # row and a later settlement row. Collapse versions only in the read projection;
        # never rewrite or discard the source ledger.
        display_by_target = {int(r["target_round"]): dict(r) for r in rows}
        # External settlement records use SETTLED while the web projection uses
        # OUTCOME_RECORDED. Normalize the display copy only so completed history
        # remains visible after a later target becomes current.
        for display_row in display_by_target.values():
            if display_row.get("result_status") == "SETTLED":
                display_row["result_status"] = "OUTCOME_RECORDED"
        display_by_target[target] = effective_row
        rows_for_display = [display_by_target[key] for key in sorted(display_by_target)]
        ...
        completed_target = max((int(r["target_round"]) for r in completed), default=None)
        latest_draw = draws[-1]
        latest_result_row = display_by_target.get(int(latest_draw["round"]))
        latest_result = None
        if latest_result_row and latest_result_row.get("result_status") == "OUTCOME_RECORDED":
            latest_result = {
                "round": latest_draw["round"], "date": latest_draw["date"],
                "main": list(latest_draw["main"]), "bonus": latest_draw["bonus"],
                "fixed": [{"label": k, "numbers": latest_result_row[f"fixed_{k}"],
                           "hits": int(latest_result_row[f"fixed_hits_{k}"])} for k in "ABC"],
                "linked": [{"label": k, "numbers": latest_result_row[f"linked_{k}"],
                            "hits": int(latest_result_row[f"linked_hits_{k}"])} for k in "ABC"],
                "fixed_primary": sum(int(latest_result_row[f"fixed_hits_{k}"]) == 3 for k in "ABC"),
                "fixed_support": sum(int(latest_result_row[f"fixed_hits_{k}"]) == 2 for k in "ABC"),
                "linked_primary": sum(int(latest_result_row[f"linked_hits_{k}"]) == 3 for k in "ABC"),
                "linked_support": sum(int(latest_result_row[f"linked_hits_{k}"]) == 2 for k in "ABC"),
            }
        next_target = draws[-1]["round"] + 1
        display_target = target if pending else next_target
        ...
        return {
            ...
            "lifecycle":{"display_target":display_target,"display_status":"PENDING" if pending else "READY_FOR_PREVIEW",
                         "completed_target":completed_target,"next_target":next_target},
            "operation":{"official_engine":"FROZEN","no_pick":"UNRESOLVED","draw_discovery_pause":"ACTIVE",
                         "verified_independent_signal":"NONE",**research},
    ...
    def record_outcome(self, target: int) -> dict[str, Any]:
        ...
        settlement_path = self.prospective_root / f"P45_TRIO_ORBIT_TARGET_{target}_SETTLEMENT_001.json"
        if outcome_path.exists() or settlement_path.exists(): raise RuntimeError("OUTCOME_ALREADY_EXISTS")
        ...
        settlement = {"target_round":target,"actual_date":draw["date"],"actual_main":list(draw["main"]),"actual_bonus":draw["bonus"],
                      "scoring_scope":"MAIN6_ONLY","sealed_record_sha256":row["selection_record_sha256"],
                      "fixed":{"sets":[list(_trio(row[f"fixed_{k}"])) for k in "ABC"],"hits":result["fixed_hits"],
                               "primary_3of3":result["fixed_exact3"],"support_exact_2of3":result["fixed_exact2"]},
                      "linked":{"sets":[list(_trio(row[f"linked_{k}"])) for k in "ABC"],"hits":result["linked_hits"],
                                "primary_3of3":result["linked_exact3"],"support_exact_2of3":result["linked_exact2"]},
                      "historical_combined":False,"future_leakage":0,"result_status":"SETTLED"}
        ...
        self._safe_write(settlement_path,json.dumps(settlement,ensure_ascii=False,indent=2)+"\n",overwrite=False)
