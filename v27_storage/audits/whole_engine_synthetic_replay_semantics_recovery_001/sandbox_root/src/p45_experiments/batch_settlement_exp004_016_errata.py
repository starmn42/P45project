...
def fair_sequence_batches(seed: int, *, batch_size: int = 256):
    """Yield 100,000 iid fair sequences of 1,237 uniform six-subsets."""
    rng = np.random.default_rng(seed)
    completed = 0
    while completed < B:
        size = min(batch_size, B - completed)
        chosen = rng.integers(0,45,size=(size,1237,6),dtype=np.int16)
        ordered = np.sort(chosen,axis=2)
        bad = np.any(ordered[:,:,1:] == ordered[:,:,:-1],axis=2)
        while np.any(bad):
            replacement = rng.integers(0,45,size=(int(bad.sum()),6),dtype=np.int16)
            chosen[bad] = replacement
            ordered_bad=np.sort(replacement,axis=1)
            bad[bad]=np.any(ordered_bad[:,1:]==ordered_bad[:,:-1],axis=1)
        draw = np.zeros((size, 1237, 45), dtype=np.bool_)
        np.put_along_axis(draw, chosen, True, axis=2)
        yield draw
        completed += size


def full_sequence_transition_p(kind: str, observed_t: float, protocol: Path) -> float:
    labels = {"lag2": "EXP005_FULL_SEQUENCE", "ending": "EXP006_FULL_SEQUENCE", "mirror": "EXP011_FULL_SEQUENCE"}
    extreme = 0
    digits = np.array([(n % 10) for n in range(1, 46)])
    if kind == "lag2":
        rng=np.random.default_rng(seed_for(protocol, labels[kind])); support=np.arange(7)
        probs=np.array([hypergeom_pmf(int(k),45,6,6) for k in support]); n=1235
        expected=n*.8; denom=math.sqrt(n*hvar(6))
        for completed in range(0,B,1000):
            size=min(1000,B-completed); totals=rng.choice(support,size=(size,n),p=probs).sum(axis=1)
            extreme += int(np.count_nonzero(np.abs((totals-expected)/denom)>=abs(observed_t)-1e-15))
        return (1+extreme)/(B+1)
    for draw in fair_sequence_batches(seed_for(protocol, labels[kind])):
        if kind == "ending":
            prev = draw[:, :-1, :]; cur = draw[:, 1:, :]
            end_present = np.stack([prev[:, :, digits == d].any(axis=2) for d in range(10)], axis=2)
            candidate = np.take(end_present, digits, axis=2) & ~prev
            z = candidate.sum(axis=2); k = (candidate & cur).sum(axis=2)
            residual = (k - 6*z/45).sum(axis=1)
            p = z/45; variance = (6*p*(1-p)*(39/44)).sum(axis=1)
            simulated_t = residual / np.sqrt(variance)
        else:
            prev = draw[:, :-1, :]; cur = draw[:, 1:, :]
            candidate = prev[:, :, ::-1].copy(); candidate[:, :, 22] = False
            z = candidate.sum(axis=2); k = (candidate & cur).sum(axis=2)
            residual = (k - 6*z/45).sum(axis=1)
            p = z/45; variance = (6*p*(1-p)*(39/44)).sum(axis=1)
            simulated_t = residual / np.sqrt(variance)
        extreme += int(np.count_nonzero(np.abs(simulated_t) >= abs(observed_t)-1e-15))
    return (1 + extreme) / (B + 1)
...
def exp005(mains: list[set[int]], protocol: Path) -> dict:
    ...
    mc = full_sequence_transition_p("lag2", t, protocol)
    ...
    wf_p = None
    if exposed:
        rng=np.random.default_rng(seed_for(protocol,"EXP005_WALKFORWARD")); support=np.arange(7)
        probs=np.array([hypergeom_pmf(int(k),45,6,6) for k in support]); extreme=0
        for completed in range(0,B,1000):
            size=min(1000,B-completed)
            null_score=(rng.choice(support,size=(size,exposed),p=probs)-.8).sum(axis=1)
            extreme += int(np.count_nonzero(null_score >= score-1e-15))
        wf_p=(1+extreme)/(B+1)
...
def exp006(mains: list[set[int]], protocol: Path) -> dict:
    ...
    mc=full_sequence_transition_p("ending",s["t"],protocol)
...
def exp007(mains: list[set[int]], protocol: Path) -> dict:
    ks=[]; zs=[]; non=0
    residual_history=[]; variance_history=[]; wf_exposed=0; wf_score=0.0
    for r in range(4,1238):
        ...
        if len(c)>=45:
            non+=1; continue
        z=len(c); k=len(c&mains[r])
        if len(residual_history) >= 200:
            train_t=sum(residual_history)/math.sqrt(sum(variance_history))
            if 2*norm_sf(abs(train_t)) <= .05:
                direction=1 if sum(residual_history)>0 else -1
                wf_exposed += 1; wf_score += direction*(k-6*z/45)
        zs.append(z); ks.append(k); residual_history.append(k-6*z/45); variance_history.append(hvar(z))
    s=martingale_summary(np.array(ks),np.array(zs))
    boot=rademacher_p(s["residual"],float(s["variance"].sum()),seed_for(protocol,"EXP007_MULTIPLIER"))
    return {k:v for k,v in s.items() if k not in {"residual","variance"}} | {"informative_rounds":len(zs),"non_informative_rounds":non,"bootstrap_p":boot,"post_minimum_evaluable":max(0,len(zs)-200),"walkforward_exposed":wf_exposed,"walkforward_score":wf_score,"walkforward_status":"NO_SIGNAL_EXPOSURE" if wf_exposed==0 else "SIGNAL_EXPOSED","judgment":"FAILED"}
...
def exp011(mains: list[set[int]], protocol: Path) -> dict:
    ...
    for r in range(2,1238):
        ...
        zs.append(len(c)); ks.append(len(c&mains[r]))
    s=martingale_summary(np.array(ks),np.array(zs)); mc=full_sequence_transition_p("mirror",s["t"],protocol)
...
def exp015(mains: list[set[int]], bonuses: list[int]) -> dict:
    ...
    training=sum(np.array(ranks[:618])-4); hold_score=int(sum(np.array(ranks[618:])-4))
    hold_dist=np.array([1.0])
    for _ in range(619): hold_dist=np.convolve(hold_dist,base)
    hold_idx=hold_score+3*619
    hold_p=float(hold_dist[:hold_idx+1].sum()) if training<0 else (float(hold_dist[hold_idx:].sum()) if training>0 else None)
    return {"target_rounds":len(ranks),"mean_rank":float(np.mean(ranks)),"rank_shift":float(np.mean(ranks)-4),"t":t,"analytical_p":float(2*norm_sf(abs(t))),"exact_p":exact,"holdout_one_sided_p":hold_p,"rank_counts":dict(sorted(Counter(ranks).items())),"judgment":"FAILED_EARLY"}
