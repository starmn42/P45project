...
def _draw_value(row:Mapping[str,Any])->tuple[int,str,tuple[int,...],int]:
    row=dict(row)
    main=row.get("main_json")
    if isinstance(main,str):main=json.loads(main)
    if main is None:main=[row[f"n{i}"] for i in range(1,7)]
    return int(row.get("draw_round",row.get("round"))),str(row.get("draw_date",row.get("date"))),tuple(map(int,main)),int(row["bonus"])

def assert_draw_sources_synchronized(db:sqlite3.Connection,live_data:Path=LIVE_DATA)->int:
    """Fail before acquisition when the two canonical live draw stores diverge."""
    with live_data.open("r",encoding="utf-8-sig",newline="") as f:
        csv_rows=[_draw_value(row) for row in csv.DictReader(f)]
    db_rows=[_draw_value(row) for row in db.execute("SELECT draw_round,draw_date,main_json,bonus FROM draw_result ORDER BY draw_round")]
    rounds=[row[0] for row in csv_rows]
    if not csv_rows or csv_rows!=db_rows or rounds!=list(range(1,rounds[-1]+1)):
        raise RuntimeError("STOP_DRAW_SOURCE_OUT_OF_SYNC")
    return rounds[-1]

def _materialize_csv(draws:Sequence[Draw],target:Path)->None:
    target.parent.mkdir(parents=True,exist_ok=True)
    ordered=sorted(draws,key=lambda x:x.round)
    if [d.round for d in ordered]!=list(range(1,ordered[-1].round+1)):raise RuntimeError("STOP_DRAW_SOURCE_OUT_OF_SYNC")
    with target.open("w",encoding="utf-8-sig",newline="") as dst:
        wr=csv.writer(dst,lineterminator="\n");wr.writerow(["round","date","n1","n2","n3","n4","n5","n6","bonus"])
        for d in ordered:wr.writerow([d.round,d.date,*d.main,d.bonus])
...
def _normalize_lifecycle_outcome(row:Mapping[str,Any])->dict[str,Any]:
    """Normalize updater/repair storage shapes without changing hit or conflict meaning."""
    out=dict(row)
    for canonical,compact in (("a_integrated_hits","ia"),("b_integrated_hits","ib"),("a_main_hits","ma"),("b_main_hits","mb")):
        if canonical not in out:
            if compact not in out:raise RuntimeError(f"PAIR_LIFECYCLE_OUTCOME_SCHEMA_INVALID:{canonical}")
            out[canonical]=int(out[compact])
    if "representative_conflict" not in out:
        if "rank_key_json" not in out:raise RuntimeError("PAIR_LIFECYCLE_OUTCOME_SCHEMA_INVALID:representative_conflict")
        rank=json.loads(out["rank_key_json"])
        if len(rank)<=8 or not isinstance(rank[8],list) or len(rank[8])<=1:raise RuntimeError("PAIR_LIFECYCLE_RANK_SCHEMA_INVALID")
        out["representative_conflict"]=int(rank[8][1])>0
    return out
...
def _history_map(draw:Draw,evaluations:Sequence[Mapping[str,Any]])->dict[str,list[dict[str,Any]]]:
    ...
    try:raw=[dict(x) for x in db.execute("SELECT s.*,o.*,c.rank_key_json FROM wf_selection_exposure s JOIN wf_outcome o USING(selection_id) JOIN wf_pair_candidate c ON c.run_id=s.run_id AND c.evaluation_round=s.evaluation_round AND c.canonical_pair_key=s.representative_pair_key ORDER BY s.evaluation_round")]
    finally:db.close()
    result:dict[str,list[dict[str,Any]]]={}
    # The repair path derives this pre-result field from the locked rank vector;
    # the operational updater must restore the same field before lifecycle reads it.
    for row in raw:
        normalized=_normalize_lifecycle_outcome(row);result.setdefault(normalized["base_pair_rule_signature"],[]).append(normalized)
    pair_evaluations=[x for x in evaluations if x["scope"]=="PAIR_DIAGNOSTIC"]
    if pair_evaluations and result:
        signature=next(iter(result));top=dict(pair_evaluations[0]);top["evaluation_round"]=draw.round
        # ProductionPairPipeline's current locked rank input sets conflict_columns=0.
        # Preserve that exact pre-result meaning; this is not a missing-value default.
        top["representative_conflict"]=False;result[signature].append(_normalize_lifecycle_outcome(top))
    return result
...
def run_update(*,fetcher=fetch_official,db_path:Path=LIVE_DB,live_data_path:Path=LIVE_DATA)->dict[str,Any]:
    db=_connect(db_path);run_id=str(uuid.uuid4());run_started=False
    try:latest=assert_draw_sources_synchronized(db,live_data_path)
    ...
    try:
        try:draw=fetcher(latest+1)
            ...
            return {"status":status,"latest_completed_draw":latest}
        ...
        db.execute("INSERT INTO update_run(run_id,started_at,stored_latest_round,status) VALUES(?,?,?,?)",(run_id,_now(),latest,"RUNNING"));db.commit();run_started=True
        ...
        with tempfile.TemporaryDirectory(dir=LIVE_DIR) as temp:
            ...
            db.execute("INSERT OR REPLACE INTO provenance VALUES(?,?)",("official_source",draw.source_url));db.commit();os.replace(published,live_data_path);assert_draw_sources_synchronized(db,live_data_path)
        ...
        db.rollback()
        if run_started:db.execute("UPDATE update_run SET completed_at=?,status=?,error_code=? WHERE run_id=?",(_now(),"DRAW_DATA_INVALID",str(exc),run_id));db.commit()
        return {"status":"DRAW_DATA_INVALID","error":str(exc),"latest_completed_draw":latest}
    except Exception as exc:
        db.rollback()
        if run_started:db.execute("UPDATE update_run SET completed_at=?,status=?,error_code=? WHERE run_id=?",(_now(),"DRAW_UPDATE_FAILED",f"{type(exc).__name__}:{exc}",run_id));db.commit()
        raise
    finally:db.close()
