...
## Draw updater lifecycle compatibility fix

- 확정 원인: updater의 임시 pair outcome은 `ia/ib/ma/mb` 축약 키였고, repaired lifecycle은 정식 `a_integrated_hits/b_integrated_hits/a_main_hits/b_main_hits`와 `representative_conflict`를 요구했다. 공식 DB에는 정식 hit 필드가 존재하며 obsolete가 아니다.
- 분류: `PAIR_REPAIR_COMPATIBILITY_GAP`.
- 수정: `src/p45_v27/draw_update.py`의 history adapter에서 축약/정식 구조를 명시적으로 정규화하고, repair와 동일하게 locked `rank_key_json`에서 conflict를 복원한다. 손상 구조는 명시 오류로 중단한다.
- source-of-truth guard: 수집 전 live CSV와 SQLite의 전체 canonical equality/연속성을 확인하고 불일치 시 `STOP_DRAW_SOURCE_OUT_OF_SYNC`.
- 1239 후처리: 1239 재삽입 없이 평가 22건과 analysis snapshot 1240을 복구했다. snapshot status `LIVE_ANALYSIS_READY`, valid_for_core `0`, prediction hash `3b9447dca2d579cc47bebe08c2e83129757a766d987ec59c89369423734a4fc2`.
- 1240 미발표 live-clone 검사: `DRAW_ALREADY_CURRENT`, draw/CSV 변화 0, duplicate 0, integrity/FK PASS.
- regression: 신규 6건 + 기존 pair shadow repair 5건 PASS.
- canonical draw는 계속 `1..1239`; CSV/SQLite draw_result 전체 equality PASS.
- Official Engine `FROZEN`; gate/threshold/signature/NUMBER/TRIO/PAIR/CORE 의미 변화 0.
